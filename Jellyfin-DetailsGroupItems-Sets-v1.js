(function () {
  ...
  function parseCollectionName(name) {
    ...
    for (const rule of CONFIG.phraseParser || []) {
      ...
      out = out.replaceAll(String(rule.from), String(rule.to || ""));
    }
  ...
  function getCredentialsServer() {
    try {
      ...
      return obj?.Servers?.find((s) => s.AccessToken) || null;
    } catch {
      return null;
    }
  }
  ...
  function getAccessToken() {
    return getCredentialsServer()?.AccessToken || null;
  }
  ...
  function getUserId() {
    ...
    return getCredentialsServer()?.UserId || null;
  }
  ...
  function findRowByLabel(box, labelText) {
    ...
    const labels = box.querySelectorAll(".detailsGroupItem .label, .label");
  ...
  async function buildCollectionIndex() {
    const cacheKey = "collectioninject_index_v8";
  ...
  async function getCollectionIndexStable() {
    if (!STATE.indexPromise) {
      STATE.indexPromise = buildCollectionIndex().catch((err) => {
  ...
  async function prepareStateForCurrentRoute() {
    ...
    log("Current item:", item?.Name, item?.Type);
    ...
    STATE.currentCollections = index[itemId] || [];
